...
import PolicyState
import State
from Parameters import alpha, beta, b_habit, eps_p, eps_w, phi_w, phi_p, pi_ss, zeta_w, zeta_p, chi, kappa, delta_0, rho_i, phi_pi, phi_y, psi, rho_A, rho_Z, rho_G, rho_nu, rho_psi, s_A, s_Z, s_G, s_nu, s_psi, s_i, s_w, omega, G_gov, F_prod, i_ss, i_LB, division_eps, eps_Ky
...
def equations(state, policy_state):
  ...
  # original eq 1
    ...
    # modification to avoid NaNs
    loss_dict['eq_9'] = (1.0 - alpha) * State.Ax(state) * mcy_norm * tf.math.maximum(Ky,eps_Ky)**alpha - wy_norm * Ny_norm**alpha    
    ...
    # modification to avoid NaNs
    loss_dict['eq_14'] = State.Ax(state) * tf.math.maximum(Ky,eps_Ky)**alpha * Ny_norm**(1.0 - alpha) - F_prod - Yy_norm * nupy_norm  
