...
if "USE_CONFIG_FROM_RUN_DIR" in os.environ.keys():
    shutil.copyfile(os.environ["USE_CONFIG_FROM_RUN_DIR"] + "/.hydra/config.yaml", "config_postprocess/config.yaml")
...
def set_conf(cfg):
    ...
    optim = getattr(tf.keras.optimizers,cfg.optimizer)(learning_rate=cfg.learning_rate * learning_rate_multiplier, clipvalue=cfg.clipvalue)
