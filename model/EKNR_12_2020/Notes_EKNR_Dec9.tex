...
\title{ NOTES }
\author{Thomas M. Mertens\thanks{\textbf{Federal Reserve Bank of San Francisco,} 101 Market Street, Mailstop 1130, San Francisco, CA 94105; Thomas.Mertens@sf.frb.org}}
\date{\today}
...
\begin{enumerate}
	\item The set of endogenous state variables has the following elements: \\
	 $K_t$\\
	 CODE: Kx (\emph{code variables: states end with 'x', controls with 'y'})
	\item The set of exogenous state variables has the following elements: \\
	 $T^D_{n,t}$, $\phi_{n,t}$, $L_{n,t}$, $d_{ni,t}$, $\chi_{n,t}$\\
	 CODE: ...
...
\begin{center}
	\begin{tabular}{ccc} 
		...
		$\theta$ () & & 2 \\ 
		$\sigma$ () & & 2.5 \\
		$\forall n$: $\omega_n$ () & & 0.5 \\
		$\beta^L$ () & & 2/3 \\
		$\beta^K$ () & & 1 - $\beta^L$ \\
		$\delta$ () & & 0.1 \\
 		$\alpha$ () & & 0.55 \\ 
		$\rho$ () & & 0.95 \\ 
		$\forall n$: $\rho_{T_n}$ () & & 0.85 \\ 
		$for \ n-1 \ countries$: $\rho_{\phi_n}$ () & & 0.85 \\ 
		$\forall n$: $\rho_{\chi_n}$ () & & 0.85 \\ 
		$\forall n \ \& \ i$: $\bar{d}_{ni}$ () & & 0.5 \\ 
		$\forall n$: $d_{nn}$ () & & 1 \\ 
		$\forall n$:  $\bar{L}_{n}$ () & & 0 \\
		\hline
	\end{tabular}
\end{center}
