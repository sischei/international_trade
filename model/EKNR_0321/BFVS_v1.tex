...
\begin{enumerate}
	...
	\item The set of exogenous state variables has the following elements (n: country index): \\
	 $T^D_{n,t}$, $T^N_{n,t}$, $T^S_{n,t}$, $\phi_{n,t}$, $\psi_{n,t}^N$ \\
	 CODE: $TD\_n\_x$, $TN\_n\_x$, $TS\_n\_x$, $phi\_n\_x$, $psi\_N\_x$ \\
	 \item The set of control variables has the following elements (n: country index): \\
	 $A_{n, t}^{D}$, $A_{n, t}^{N}$, $A_{n, t}^{S}$, $\psi_{n,t}^S$, $p^{h}_{n,t}$,
	 $C^h_{n,t}$, $r_{n,t}$, $K^{H,D}_{n,t}$, $Y^D_{n,t}$, $Y^S_{n,t}$, $Y^N_{n,t}$, $w_{n,t}$, \\
	 CODE: $AD\_n\_y$ ($>0$), $AN\_n\_y$ ($>0$), $AS\_n\_y$ ($>0$), 
	 $psi\_S\_n\_y$ ($>0$), $p\_h\_n\_y$ ($>0$), $C\_h\_n\_y$ ($>0$), $r\_n\_y$ ($>0$), $K\_HD\_n\_y$ ($>0$), $YD\_n\_y$ ($>0$), $YS\_n\_y$ ($>0$), $YN\_n\_y$ ($>0$), $w\_n_\_y$ ($>0$) \\
\end{enumerate}
...
\noindent {\bf{$N \times 2$} ??? N?} equations. Household spending on consumption good of sector $S$ is (in code: {\bf{hh\_spending\_S}}) 

\begin{equation}
0 = \cl{p^h_{n,t}} \cl{C^h_{n,t}} - \omega_n \st{\phi_{n,t}} \cl{\psi_{n,t}^S}
\end{equation}
and for sector $N$ accordingly (in code: {\bf{hh\_spending\_N}})
\begin{equation}
0 = \cl{p^h_{n,t}} \cl{C^h_{n,t}} - \omega_n \st{\phi_{n,t}} \st{\psi_{n,t}^N}
\end{equation}
...
\noindent $N \times 3$ equations. The cost of a bundle of factors
\begin{equation}
...
\noindent $N \times 2$ equations. The price index of the tradable sectors $l \in \{D,N\}$
\begin{equation}
0 = \cl{p^l_{n,t}} - \left(\sum^{N}_{i=1} \left( \frac{\cl{c^l_{i,t}} \st{d^l_{ni,t}}}{\cl{A_{i,t}^l}} \right)^{-\theta} \right)^{-\frac{1}{\theta}}
\end{equation}
The fraction of goods from tradable sector $l \in \{D,N\}$ that country $n$ obtains as imports from country $i$
\begin{equation}
...
\noindent $N \times 2$ equations. The gross production of the tradable good in sector $l \in \{D,N\}$ in a country equals
\begin{equation}
...
\end{equation}
\noindent $N \times (N-1) \times 2$ equations for $l \in \{S,N\}$
\begin{equation}
